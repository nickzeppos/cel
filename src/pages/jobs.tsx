...
const Jobs: NextPage = () => {
  ...
  const [didFetchTermQueueState, setDidFetchTermQueueState] = useState(false)
  const fetchTermTimeoutRef = useRef<number | null>(null)
  ...
  const termQueueState = trpc.useQuery(['term-queue.state'], {
    onSuccess() {
      if (fetchTermTimeoutRef.current != null) {
        window.clearInterval(fetchTermTimeoutRef.current)
        fetchTermTimeoutRef.current = null
      }
      setDidFetchTermQueueState(false)
      window.setTimeout(() => {
        setDidFetchTermQueueState(true)
      }, 0)
    },
    refetchOnWindowFocus: false,
  })
  ...
  const debouncedTermRefetch = useDebounce(termQueueState.refetch, 100, {
    leading: true,
    trailing: true,
    maxWait: 500,
  })
  ...
  const pauseTermQueue = trpc.useMutation(['term-queue.pause'])
  ...
  return (
    <div>
      ...
      >
        <div className="flex items-center gap-2 p-4 pb-0">
          ...
          <PlusCircleIconSolid
            ...
            onClick={() => {
              addBillJob.mutate(
                billJobDataValidator.parse({ billNum, congress, billType }),
              )
            }}
          />
          {isBillQueuePaused ? (
      ...
      <div
        className={`m-4 rounded-t-xl border-4 ${
          isTermQueuePaused ? 'border-red-400/60' : 'border-green-400/60'
        }`}
      >
        <div className="flex items-center gap-2 p-4 pb-0">
          ...
          <TrashIconSolid
            className="h-8 w-8 cursor-pointer text-white"
            onClick={() => {
              cleanTermQueue.mutate()
            }}
          />
          <div className="text-xl font-bold">
            {termQueueName} (
            {
              termQueueJobbies.filter(
                (j) =>
                  j.state === 'active' ||
                  j.state === 'delayed' ||
                  j.state === 'waiting',
